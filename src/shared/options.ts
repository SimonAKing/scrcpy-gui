...
export const OPTION_DESCRIPTORS: readonly OptionDescriptor[] = [
  ...
  { key: 'videoCodec', flag: '--video-codec', category: 'video', valueType: 'enum', defaultValue: 'default', helpKey: 'codec', minScrcpyVersion: '4.0', scenes: visual, serialize: (c) => c.videoCodec === 'default' ? [] : [`--video-codec=${c.videoCodec}`], validate: ok },
  { key: 'videoBitRate', flag: '--video-bit-rate', aliases: ['-b'], category: 'video', valueType: 'number', defaultValue: 8, helpKey: 'bitRate', minScrcpyVersion: '4.0', scenes: visual, serialize: (c) => c.videoBitRate > 0 && c.videoBitRate !== 8 ? [`--video-bit-rate=${c.videoBitRate}M`] : [], validate: ok },
  ...
  { key: 'audioBuffer', flag: '--audio-buffer', category: 'video', valueType: 'number', defaultValue: 0, helpKey: 'audioBuffer', minScrcpyVersion: '4.0', scenes: visual, serialize: (c) => c.audioBuffer > 0 ? [`--audio-buffer=${Math.trunc(c.audioBuffer)}`] : [], validate: ok },
  { key: 'maxSize', flag: '--max-size', aliases: ['-m'], category: 'video', valueType: 'number', defaultValue: 0, helpKey: 'maxSize', minScrcpyVersion: '4.0', scenes: visual, serialize: (c) => c.maxSize > 0 ? [`--max-size=${Math.trunc(c.maxSize)}`] : [], validate: ok },
  { key: 'maxFps', flag: '--max-fps', category: 'video', valueType: 'number', defaultValue: 0, helpKey: 'maxFps', minScrcpyVersion: '4.0', scenes: screenVirtualRecord, serialize: (c) => c.maxFps > 0 ? [`--max-fps=${Math.trunc(c.maxFps)}`] : [], validate: ok },
  ...
  { key: 'orientation', flag: '--orientation', category: 'video', valueType: 'enum', defaultValue: '0', helpKey: 'orientation', minScrcpyVersion: '4.0', scenes: visual, serialize: (c) => c.orientation === '0' ? [] : [`--orientation=${c.orientation}`], validate: ok },
